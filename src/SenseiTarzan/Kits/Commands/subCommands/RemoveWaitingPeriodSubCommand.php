...
class RemoveWaitingPeriodSubCommand extends BaseSubCommand
{
	...
	public function onRun(CommandSender $sender, string $aliasUsed, array $args) : void
	{
		...
		$kit = $args["kit"];
		if ($kit === null) {
			$sender->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($sender, CustomKnownTranslationFactory::error_no_exist_kit("?")));
			return;
		}

		$kitsPlayer = KitsPlayerManager::getInstance()->getPlayer($player);
		if ($kitsPlayer === null) {
			$sender->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($sender, CustomKnownTranslationFactory::error_not_found_kits_player_admin($player)));
			return;
		}
		if (!$kitsPlayer->hasWaitingPeriod($kit->getId())) {
			$sender->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($sender, CustomKnownTranslationFactory::error_not_found_waiting_period($player, $kit)));
			return;
		}

		Await::g2c($kitsPlayer->removeWaitingPeriod($kit->getId()), function () use ($sender, $player, $kit){
			$sender->sendMessage(Main::getInstance()->getLanguageManager()->getTranslateWithTranslatable($sender, CustomKnownTranslationFactory::success_remove_waiting_period($player, $kit)));
		}, fn () => null);
